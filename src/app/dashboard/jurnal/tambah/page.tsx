...
export default function TambahJurnalPage() {
  ...
  const { register, handleSubmit, control, formState: { errors } } = useForm<FV>({ resolver: zodResolver(schema), defaultValues: { tanggal: today, items: [{ akun_id: '', debit: 0, credit: 0 }] } })
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center gap-4"><Button variant="ghost" size="icon" asChild><a href="/dashboard/jurnal"><ArrowLeft className="h-5 w-5" /></a></Button><div><h1 className="text-3xl font-heading font-bold">Tambah Jurnal</h1></div></div>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <CardContent className="space-y-4">{fields.map((f, i) => (
            <div key={f.id} className="p-4 border rounded-lg space-y-3"><div className="flex items-center justify-between"><span className="text-sm text-muted-foreground">Item #{i+1}</span>{fields.length > 1 && <Button type="button" variant="ghost" size="sm" onClick={() => remove(i)}><Trash2 className="h-4 w-4 text-destructive" /></Button>}</div>
              <div className="grid grid-cols-4 gap-3">
                <div className="space-y-2"><label className="text-xs font-medium">Akun *</label>
                  <select {...register(`items.${i}.akun_id`)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring"><option value="">Pilih</option>{akunOpts.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div>
        ...
        <div className="flex justify-end gap-3"><Button type="button" variant="outline" asChild><a href="/dashboard/jurnal">Batal</a></Button>
