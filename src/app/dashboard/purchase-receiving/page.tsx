  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button asChild><Link href="/dashboard/purchase-receiving/tambah"><Plus className="h-4 w-4 mr-2" />Tambah Receiving</Link></Button>
      </div>
      ...
      <div className="rounded-lg border bg-card"><table className="w-full"><thead><tr className="border-b bg-muted/50">
        ...
        <th className="text-left p-3 text-xs font-medium text-muted-foreground uppercase">PO Ref</th>
        ...
        {data.map((item) => (
          <tr key={item.id} className="hover:bg-muted/30">
            ...
            <td className="p-3 text-sm text-muted-foreground">{item.purchase_order?.nomor ?? '-'}</td>
            ...
            <td className="p-3 text-right"><Button variant="ghost" size="sm" asChild><Link href={`/dashboard/purchase-receiving/${item.id}/edit`}><Pencil className="h-4 w-4" /></Link></Button></td>
          </tr>
        ))}
      </tbody></table></div>}
    </div>
