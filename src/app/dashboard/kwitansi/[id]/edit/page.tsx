...
export default function EditKwitansiPage() {
  ...
  useEffect(() => {
    apiFetch<{ status: string; keterangan: string | null }>(`/api/v1/kwitansi/${params.id}`)
      .then(r => { reset({ status: r.data.status, keterangan: r.data.keterangan ?? '' }); setLoading(false) }).catch(() => { toast.error('Gagal'); router.push('/dashboard/kwitansi') })
  }, [params.id, reset, router])
  const onSubmit = async (data: FV) => {
    setSubmitting(true); try { await apiFetch(`/api/v1/kwitansi/${params.id}`, { method: 'PUT', body: JSON.stringify(data) }); toast.success('Diupdate!'); router.push('/dashboard/kwitansi') }
    catch (err) { toast.error(err instanceof Error ? err.message : 'Error') } finally { setSubmitting(false) }
  }
  if (loading) return <div className="flex justify-center py-12"><Loader2 className="h-6 w-6 animate-spin" /></div>
  return (
    <div className="max-w-xl space-y-6"><div className="flex items-center gap-4"><Button variant="ghost" size="icon" asChild><Link href="/dashboard/kwitansi"><ArrowLeft className="h-5 w-5" /></Link></Button><div><h1 className="text-3xl font-heading font-bold">Edit Kwitansi</h1></div></div>
      ...
      </CardContent></Card><div className="flex justify-end gap-3"><Button type="button" className="bg-red-500/70 text-white hover:bg-red-500/90" asChild><Link href="/dashboard/kwitansi">Batal</Link></Button><Button type="submit" disabled={submitting}>{submitting ? '...' : 'Update'}</Button></div></form>
    </div>
  )
}
