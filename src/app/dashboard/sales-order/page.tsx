...
export default async function SalesOrderPage() {
  const { data, error } = await supabase.from('sales_order').select('*, customer_po!customer_po_id(nomor)').order('created_at', { ascending: false })
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border bg-card"><table className="w-full"><thead><tr className="border-b bg-muted/50">
        ...
        <th className="text-left p-3 text-xs font-medium text-muted-foreground uppercase">PO Reference</th>
        ...
        {data.map((item) => (
          <tr key={item.id} className="hover:bg-muted/30">
            ...
            <td className="p-3 text-sm text-muted-foreground">{item.customer_po?.nomor ?? '-'}</td>
            ...
            <td className="p-3 text-right"><Button variant="ghost" size="sm" asChild><Link href={`/dashboard/sales-order/${item.id}/edit`}><Pencil className="h-4 w-4" /></Link></Button></td>
          </tr>
        ))}
      </tbody></table></div>}
    </div>
