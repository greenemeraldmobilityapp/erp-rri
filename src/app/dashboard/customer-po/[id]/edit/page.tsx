...
export default function EditPoPage() {
  ...
  const onSubmit = async (data: FV) => {
    setSubmitting(true); try {
      const res = await apiFetch<{ autoGenerated?: { id: string; nomor: string; type: string } }>(`/api/v1/customer-po/${params.id}`, { method: 'PUT', body: JSON.stringify(data) })
      const autoGenerated = res.data?.autoGenerated
      if (autoGenerated) {
        toast.success('PO diupdate!', { description: `Sales Order ${autoGenerated.nomor} otomatis dibuat`, action: { label: 'Lihat SO', onClick: () => router.push(`/dashboard/sales-order/${autoGenerated.id}`) } })
        return
      }
