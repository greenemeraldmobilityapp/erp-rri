  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border bg-card"><table className="w-full"><thead><tr className="border-b bg-muted/50">
        ...
        <th className="text-left p-3 text-xs font-medium text-muted-foreground uppercase">PO Customer</th>
        ...
        {data.map((item) => (
          <tr key={item.id} className="hover:bg-muted/30">
            ...
            <td className="p-3 text-sm text-muted-foreground">{item.nomor_po_customer ?? '-'}</td>
            ...
            <td className="p-3 text-right"><Button variant="ghost" size="sm" asChild><Link href={`/dashboard/customer-po/${item.id}/edit`}><Pencil className="h-4 w-4" /></Link></Button></td>
          </tr>
        ))}
      </tbody></table></div>}
    </div>
