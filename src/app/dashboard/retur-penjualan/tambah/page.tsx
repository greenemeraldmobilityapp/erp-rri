...
export default function TambahReturPenjualanPage() {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center gap-4"><Button variant="ghost" size="icon" asChild><a href="/dashboard/retur-penjualan"><ArrowLeft className="h-5 w-5" /></a></Button>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <Card><CardHeader><CardTitle className="text-base">Informasi</CardTitle></CardHeader><CardContent className="space-y-4">
          ...
          <div className="space-y-2"><label className="text-sm font-medium">DO Reference</label>
            <select {...register('delivery_order_id')} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring"><option value="">Pilih DO</option>{doOpts.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div>
        ...
        <div className="flex justify-end gap-3"><Button type="button" variant="outline" asChild><a href="/dashboard/retur-penjualan">Batal</a></Button>
