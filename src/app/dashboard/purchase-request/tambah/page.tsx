...
export default function TambahPrPage() {
  ...
  useEffect(() => {
    Promise.all([apiFetch<Array<{ id: string; nomor: string }>>('/api/v1/sales-order'), apiFetch<Array<{ id: string; nama: string; kode: string }>>('/api/v1/master/barang/dropdown')])
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center gap-4"><Button variant="ghost" size="icon" asChild><Link href="/dashboard/purchase-request"><ArrowLeft className="h-5 w-5" /></Link></Button>
      ...
      <Form {...form}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex justify-end gap-3"><Button type="button" variant="cancel"><Link href="/dashboard/purchase-request">Batal</Link></Button>
