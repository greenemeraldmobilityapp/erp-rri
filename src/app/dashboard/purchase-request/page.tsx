      ...
      <div className="rounded-lg border bg-card"><Table><TableHeader><TableRow>
        ...
        {data.map((item) => (
          <TableRow key={item.id}>
            ...
            <TableCell className="text-right"><Button variant="ghost" size="sm" asChild><Link href={`/dashboard/purchase-request/${item.id}/edit`}><Pencil className="h-4 w-4" /></Link></Button></TableCell>
          </TableRow>
        ))}
      </TableBody></Table></div>}
    </div>
