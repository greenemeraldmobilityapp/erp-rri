...
export default function EditNegoiasiPage() {
  ...
  useEffect(() => {
    apiFetch<{ id: string; status: string; keterangan: string | null }>(`/api/v1/negoiasi/${params.id}`)
      ...
      .catch(() => { toast.error('Gagal memuat'); router.push('/dashboard/negoiasi') })
  }, [params.id, reset, router])

  const onSubmit = async (data: FV) => {
    setSubmitting(true); try { await apiFetch(`/api/v1/negoiasi/${params.id}`, { method: 'PUT', body: JSON.stringify(data) }); toast.success('Diupdate!'); router.push('/dashboard/negoiasi') }
  ...
  return (
    <div className="max-w-xl space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-3"><Button type="button" variant="cancel"><Link href="/dashboard/negoiasi">Batal</Link></Button>
