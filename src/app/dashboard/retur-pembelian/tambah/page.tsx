...
export default function TambahReturPage() {
  const router = useRouter(); const [poOpts, setPoOpts] = useState<Array<{ value: string; label: string }>>([]); const [supOpts, setSupOpts] = useState<Array<{ value: string; label: string }>>([]); const [barangOpts, setBarangOpts] = useState<Array<{ value: string; label: string }>>([]); const [submitting, setSubmitting] = useState(false)
  ...
  const onSubmit = async (data: FV) => {
    setSubmitting(true); try { await apiFetch('/api/v1/retur-pembelian', { method: 'POST', body: JSON.stringify(data) }); toast.success('Retur berhasil!'); router.push('/dashboard/retur-pembelian') }
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <Card><CardHeader><CardTitle className="text-base">Informasi</CardTitle></CardHeader><CardContent className="space-y-4">
          ...
          <div className="space-y-2"><label className="text-sm font-medium">PO Reference</label>
